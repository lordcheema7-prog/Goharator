...
The game works immediately in offline-first mode: pilot profiles, scores, and settings are stored in the browser. The optional `server.js` API uses local JSON files and should be deployed separately on a stateful Node host if you need a shared global leaderboard or cross-device accounts. Do not deploy it as a Vercel function expecting its JSON files to persist.
